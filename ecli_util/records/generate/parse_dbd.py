...
columns = ['field', 'asl', 'initial', 'promptgroup', 'prompt', 'special', 'pp', 'interest', 'base', 'size', 'extra', 'menu']
path = 'output'
for rtype, info in record_info.iteritems():
    with open(os.path.join(path, '%s.txt' % (rtype, )), 'wt') as f:
        print('\t'.join(columns), file=f)
        fields = info['fields']
        for name, field in sorted(fields.iteritems()):
            row = [field[col] if col in field
                                else ''
                                for col in columns]
            row[0] = name
